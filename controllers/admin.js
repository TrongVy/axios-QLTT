...
function xemNguoiDung(id) {
    danhSachND.xemND(id).then(function (result) {
        ...
        var modalFooter = document.querySelector('.modal-footer');
        modalFooter.innerHTML = `<button class="btn btn-success" onclick="capNhatND('${result.data.id}')" >cap nhat</button>`
...
function showData(arr) {
    ...
    arr.map(function (arrItem, index) {
        content += `
                ...
                <td>
                <button class="btn btn-success" onclick="xemNguoiDung(${arrItem.id})">Sua</button>
                <button  class="btn btn-danger" onclick="xoaNguoiDung(${arrItem.id})">Xoa</button>
                </td>          
                </tr>
            `
    })
